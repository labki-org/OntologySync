#!/bin/bash
#
# Run OntologySync tests inside Docker MediaWiki environment.
#
...
#   ./tests/scripts/run-docker-tests.sh unit --filter OntologySyncHooks
...
EXT_PATH="/mw-user-extensions/OntologySync"

if [[ "$TEST_TYPE" == "unit" ]]; then
    echo "Running OntologySync UNIT tests..."
    echo ""
    docker compose exec -T wiki php "$EXT_PATH/vendor/bin/phpunit" \
        --configuration "$EXT_PATH/tests/phpunit/unit.xml" \
        "$@"
else
    echo "Running OntologySync INTEGRATION tests..."
